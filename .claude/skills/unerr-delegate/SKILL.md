...
## Phase D3 — Pick the tier, then spawn one worker PER group (in parallel).
  Choose the model tier by class: tests / mechanical_refactor → WORKER model; lint_format / docs / recon → JUNIOR model (cheapest).
  ...
  - Claude Code: spawn the model-pinned sub-agent per group, ALL IN ONE message so they run concurrently — `Task({subagent_type:'unerr-worker', …})` for the WORKER tier (Sonnet: tests / mechanical_refactor) or `Task({subagent_type:'unerr-junior', …})` for the JUNIOR tier (Haiku: lint_format / docs / recon). Prompt = `'<the group's recon digest>\n\nTask: <verbatim task for this group>'`. The tier model is pinned by the sub-agent's `.claude/agents/*.md` frontmatter.
