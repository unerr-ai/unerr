...
- `Task({subagent_type:'unerr-reviewer', …})` (Claude Code only) — post-edit read-only review of the working diff: correctness, missed callers via blast radius, convention violations; spawn after a multi-file or multi-agent change, before reporting done.
