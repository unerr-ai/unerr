...
export function MaskingTooltip({ reason, onUnmask }: MaskingTooltipProps) {
  ...
  async function handleUnmask() {
    setUnmasking(true);
    try {
      await fetchJson(`/api/router/unmask/${reason.family}`, { method: "POST" });
        ...
        <div className="absolute z-50 mt-1 w-72 rounded-lg border border-zinc-700 bg-zinc-900 p-4 shadow-xl text-left">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div>
              ...
              <span className="font-mono text-red-400 tabular-nums">{scorePct}%</span>
              <span className="text-zinc-600 mx-1">|</span>
              <span className="text-zinc-500">Threshold:</span>{" "}
              <span className="font-mono text-amber-400 tabular-nums">{thresholdPct}%</span>
              <span className="text-zinc-600 ml-2">(gap: {gap}pt)</span>
            </div>

            <div className="h-2 w-full rounded-full bg-zinc-800 relative">
              <div className="h-2 rounded-full bg-red-500/60" style={{ width: `${scorePct}%` }} />
              <div className="absolute top-0 h-2 w-0.5 bg-amber-400" style={{ left: `${thresholdPct}%` }} />
            </div>
            ...
            <p className="text-[10px] text-zinc-600">
              Or run: <code className="text-zinc-400">unerr router unmask {reason.family}</code>
            </p>
          </div>
        </div>
      )}
