...
describe("evaluateCondition: And/Or composites", () => {
	it("And — true only when every child true", () => {
		...
		s.advanceTurn();
		s.advanceTurn();
		expect(evaluateCondition(C.and(C.turns(3), C.nonTrivial()), s)).toBe(
			false,
		);
		s.recordCall({ toolName: "file_read", editOrWrite: true });
		expect(evaluateCondition(C.and(C.turns(3), C.nonTrivial()), s)).toBe(
...
describe("evaluateUnlocks: only newly-firing tools", () => {
	...
	it("emits one event per newly-satisfied policy", () => {
		...
		// Both gates fire on the first file read: get_project_stats AND
		// get_conventions are policy C.firstRead(). Order is sorted.
		expect(tools).toEqual(["get_conventions", "get_project_stats"]);
	});

	it("does not re-emit tools that are already exposed", () => {
		const s = new SessionState();
		s.expose(["get_project_stats"]);
		s.recordCall({ toolName: "file_read", filePath: "src/x/a.ts" });
		const events = evaluateUnlocks(s);
		expect(events.find((e) => e.toolName === "get_project_stats"))
			.toBeUndefined();
	});
	...
	it("attaches a non-empty reason and the current turn", () => {
		...
		s.recordCall({ toolName: "file_read", filePath: "src/x/a.ts" });
		const events = evaluateUnlocks(s);
		// get_project_stats is policy C.firstRead() — fires after one file
		// access, which is the signal recorded above. Any firing event has
		// the standard shape.
		const event = events.find((e) => e.toolName === "get_project_stats");
