...
describe("uninstall", () => {
  ...
  describe("--strip-annotations flag (SC-B.5)", () => {
    it("strips @sem sentinel lines repo-wide when the flag is passed", async () => {
      ...
      const after = readFileSync(join(tmpDir, "svc.ts"), "utf8");
      expect(after).not.toContain("@sem");
      expect(after).toContain("Does the thing.");
    });
    ...
    it("leaves source untouched without the flag", async () => {
      ...
      try {
        process.chdir(tmpDir);
        await program.parseAsync(["node", "unerr", "uninstall"]);
      } finally {
        process.chdir(cwd);
      }
      expect(readFileSync(join(tmpDir, "k.ts"), "utf8")).toBe(src);
    });
  });
});
