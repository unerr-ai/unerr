...
import { createStickinessState, recordFamilyCall, advanceTurn } from "../router/intent/stickiness.js";
import { buildDecayState, createEmptyDecayState } from "../router/intent/threshold-decay.js";
...
describe("Intent Scorer — DB-only scenarios", () => {
  it("entity importing pg scores pg family ≥0.40", () => {
    ...
    expect(getScore(result, "pg").score).toBeGreaterThanOrEqual(0.40);
  ...
  it("multiple DB files don't inflate score beyond entity tag weight", () => {
    const result = scoreIntent(baseInput({
      recentFiles: ["db/schema.sql", "db/migrations/001.sql", "db/seeds/data.ts"],
    }));
...
describe("Intent Scorer — Stickiness scenarios", () => {
  ...
  it("sticky family includes reason", () => {
    ...
    const result = scoreIntent(baseInput({ stickinessState: state }));
    expect(getScore(result, "pg").reasons.some((r) => r.includes("sticky"))).toBe(true);
  });
...
describe("Intent Scorer — Threshold decay scenarios", () => {
  it("never-used family requires higher score", () => {
    ...
    expect(getScore(result, "slk").thresholdApplied).toBeGreaterThan(0.30);
    expect(getScore(result, "slk").exposed).toBe(false);
  });
  ...
  it("frequently-used family has lower threshold", () => {
    ...
    const result = scoreIntent(baseInput({ decayState: decay }));
    expect(getScore(result, "pg").thresholdApplied).toBeLessThan(0.30);
  });
...
describe("Intent Scorer — Tool history signal", () => {
  it("dominant tool family gets scored via history", () => {
    ...
    expect(getScore(result, "pg").score).toBeGreaterThan(getScore(result, "gh").score);
  });
...
describe("Intent Scorer — Edge cases", () => {
  ...
  it("budget exceeded flag triggers fail-open", () => {
    const input = baseInput({
      entityFamilyTags: new Map(
        Array.from({ length: 1000 }, (_, i) => [`Entity${i}`, new Set(["pg"])] as const),
      ),
      recentFiles: Array.from({ length: 100 }, (_, i) => `db/migrations/${i}.sql`),
    });
...
describe("Intent Scorer — Signal collector integration", () => {
  it("collector builds correct scorer input shape", () => {
    ...
    expect(getScore(result, "pg").exposed).toBe(true);
    expect(getScore(result, "pg").score).toBeGreaterThanOrEqual(0.30);
  });
...
describe("Intent Scorer — Verification gate (50 scenarios)", () => {
  const scenarios: { name: string; input: Partial<ScorerInput>; expectExposed: string[]; expectNotExposed?: string[] }[] = [
    { name: "pure DB: SQL file", input: { recentFiles: ["db/schema.sql"] }, expectExposed: ["pg"] },
    { name: "pure DB: migrations dir", input: { recentFiles: ["db/migrations/001.ts"] }, expectExposed: ["pg"] },
    { name: "pure DB: Prisma entity", input: { entityFamilyTags: new Map([["PrismaClient", new Set(["pg"])]]) }, expectExposed: ["pg"] },
    { name: "pure GH: workflow", input: { recentFiles: [".github/workflows/ci.yml"] }, expectExposed: ["gh"] },
    { name: "pure GH: CODEOWNERS", input: { recentFiles: [".github/CODEOWNERS"] }, expectExposed: ["gh"] },
    { name: "pure GH: octokit entity", input: { entityFamilyTags: new Map([["GitClient", new Set(["gh"])]]) }, expectExposed: ["gh"] },
    { name: "pure Slack: slack dir", input: { recentFiles: ["src/integrations/slack/bot.ts"] }, expectExposed: ["slk"] },
    { name: "pure AWS: entity", input: { entityFamilyTags: new Map([["S3Client", new Set(["aws"])]]) }, expectExposed: ["aws"] },
    { name: "pure Stripe: entity", input: { entityFamilyTags: new Map([["PaymentService", new Set(["str"])]]) }, expectExposed: ["str"] },
    { name: "pure Redis: entity", input: { entityFamilyTags: new Map([["CacheService", new Set(["rds"])]]) }, expectExposed: ["rds"] },
    { name: "multi: DB + GH entities", input: { entityFamilyTags: new Map([["Repo", new Set(["pg"])], ["PR", new Set(["gh"])]]) }, expectExposed: ["pg", "gh"] },
    { name: "multi: DB + GH files", input: { recentFiles: ["db/schema.sql", ".github/workflows/ci.yml"] }, expectExposed: ["pg", "gh"] },
    { name: "multi: three entities", input: { entityFamilyTags: new Map([["A", new Set(["pg"])], ["B", new Set(["gh"])], ["C", new Set(["slk"])]]) }, expectExposed: ["pg", "gh", "slk"] },
    { name: "ambiguous: only README", input: { recentFiles: ["README.md"] }, expectExposed: [], expectNotExposed: ["pg", "gh"] },
    { name: "ambiguous: generic TS", input: { recentFiles: ["src/utils/helpers.ts"] }, expectExposed: [], expectNotExposed: ["pg", "gh"] },
    { name: "ambiguous: package.json", input: { recentFiles: ["package.json"] }, expectExposed: [], expectNotExposed: ["pg", "gh"] },
    { name: "vague: empty signals", input: {}, expectExposed: [] },
    { name: "tool history only: pg dominant", input: { recentToolFamilies: ["pg", "pg", "pg", "pg"] }, expectExposed: ["pg"] },
    { name: "tool history only: gh dominant", input: { recentToolFamilies: ["gh", "gh", "gh", "gh"] }, expectExposed: ["gh"] },
    { name: "tool history: equal split", input: { recentToolFamilies: ["pg", "gh", "pg", "gh"] }, expectExposed: ["pg", "gh"] },
    { name: "file + entity agree on pg", input: { recentFiles: ["db/schema.sql"], entityFamilyTags: new Map([["UserRepo", new Set(["pg"])]]) }, expectExposed: ["pg"] },
    { name: "file + entity disagree (multi-domain)", input: { recentFiles: ["db/schema.sql"], entityFamilyTags: new Map([["PRService", new Set(["gh"])]]) }, expectExposed: ["pg", "gh"] },
    { name: "Kubernetes file", input: { recentFiles: ["k8s/deployment.yaml"] }, expectExposed: [] },
    { name: "Docker file alone", input: { recentFiles: ["Dockerfile"] }, expectExposed: [] },
    { name: "vercel.json", input: { recentFiles: ["vercel.json"] }, expectExposed: [] },
  ];
  ...
  for (const scenario of scenarios) {
    it(scenario.name, () => {
      ...
      for (const family of scenario.expectExposed) {
        expect(result.exposedFamilies.has(family), `Expected ${family} to be exposed in "${scenario.name}"`).toBe(true);
      }
      if (scenario.expectNotExposed) {
        for (const family of scenario.expectNotExposed) {
          expect(result.exposedFamilies.has(family), `Expected ${family} NOT exposed in "${scenario.name}"`).toBe(false);
        }
      }
    });
  }
