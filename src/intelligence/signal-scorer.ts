    ...
    if (raw.durability_warning) {
      signals.push(
        createSignal({
          ...
          action:
            "propose an alternative approach before retrying — a high revert rate signals the prior approach won't stick",
          actionability: 0.85,
          relevance: 0.8,
          confidence: 0.75,
          source: "temporal",
      ...
      for (const fact of raw.relevant_facts.slice(0, 3)) {
        ...
        signals.push(
          createSignal({
            ...
            action: isEpisodic
              ? "read narrative above before editing if diverging from the prior intent"
              : isConvention
