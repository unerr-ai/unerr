...
const preReadHandler: HookHandler = (normalized) => {
  ...
  if (isClaudeCode) {
    ...
    return nudge(
      `READ ROUTING: Built-in Read is ONLY for the Edit workflow (Read → Edit). ` +
        `Use offset/limit to read only the lines you plan to edit — do NOT read the entire file.\n` +
...
const preEditHandler: HookHandler = (normalized) => {
  ...
  const readPrereq = isClaudeCode
    ? `CRITICAL: Edit REQUIRES built-in Read to have been called on "${filePath}" first. ` +
      `file_read (MCP) does NOT satisfy this — the Edit tool will fail with "File has not been read yet". ` +
      `If you haven't called built-in Read (with offset/limit on the target lines) on this file, do so now before attempting Edit.\n\n`
    : "";
