...
export const USING_UNERR_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    '  Preferred one-shot: run `unerr recon "<verbatim user prompt>"` from Bash. One call returns anchored notes + matching entities + the focus entity\'s callers (blast radius) + conventions, ranked and budget-trimmed — it replaces the `search_code` → `get_references` → `get_conventions` fan-out (each separate call re-bills the whole prefix). Act on the bundle. Stop and reassess if it reports fan-in > 10 or flags a large sweep.',
    "  Fallback (recon unavailable, or you need a wider slice than the bundle): call `search_code({query:'<target_symbol>'})` to locate the entity, then `get_references({entity:'<symbol>', direction:'callers'})` for every export you will touch.",
    '  Trivial / read-only lookup: skip `unerr recon` and skip the markers + `unerr_turn_summary` ceremony below — call `search_code` or `file_read` directly. Large sweep (rename / migrate / "every place that…"): run `unerr recon` inside a Task subagent and return only its digest to the main thread.',
    "",
    ...
    "  Run the Review phase (`unerr-review`, phases R4–R7) on every entity changed this turn: `get_references({key:'<entity>', direction:'callers'})` for breaking callers, `get_conventions({file_path:'<file>'})` for boundary/convention breaches, `get_test_coverage({entity:'<entity>'})` for untested changed exports. Fix critical + high before close-out. Skip this phase only on turns that changed no code.",
    "",
    "Phase 7 — Close out (REQUIRED on every coding turn).",
...
export const SAFE_MODIFICATION_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "  Run the Review phase (`unerr-review`, phases R4–R7) on every entity changed this turn: `get_references({key:'<entity>', direction:'callers'})` for the breaking-caller cascade, `get_conventions({file_path:'<file>'})` for boundary/convention breaches, `get_test_coverage({entity:'<entity>'})` for untested changed exports, `search_code({query:'<new-name>'})` for duplicate/hallucinated APIs. Judge over diff + that evidence; tag findings critical/high/medium/low. Fix critical + high before close-out. An evidenced-clean change set is a valid pass — say so.",
    "",
    "## Red Flags",
    "",
...
export const MEMORY_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "  Call: `unerr_remember({type:'note', note:'<DSL wire>'})` — the server fills session_id automatically; you do NOT supply one.",
    "",
    "## DSL vocabulary",
    "",
  ...
  category: "behavior",
  trigger: { type: "always" },
  tools: ["unerr_recall_notes", "unerr_remember", "unerr_track"],
  version: "1.0.0",
};
