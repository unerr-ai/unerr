...
export const USING_UNERR_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "",
    "**Surface 3 — End of every coding turn.**",
    "  Call `unerr_turn_summary({})` and include the returned `line` field VERBATIM in your closing message. The tool returns a single prose line like `unerr » your session: saved 2,400 tokens via 7 recalled notes, 3 code lookups · kept ~3 extra turns of chat room` — do NOT compose your own version, do NOT translate it, do NOT add markdown formatting. Just paste the `line` value as plain text.",
    "",
    "**Surface 4a — Attribution (when a fact shaped your answer).**",
    "  State it plainly inline: `unerr reminded me you'd asked to <verbatim user rule>` or `unerr says this file follows <convention>` or `unerr says <N> places call <name>`. One short clause, never tool JSON.",
    "",
    "**Surface 4b — Capture confirmation (after `unerr_remember` succeeds).**",
    "  Say: `added that to unerr for next time`. One line, after the capture.",
    "",
    "**Surface 4c — Ambiguity (when `unerr_remember` returns `please confirm`).**",
    "  Ask the user verbatim: `should I remember: '<verbatim quote>'? (yes/no)`. Do not paraphrase the quote.",
    "",
    '**Surface 4d — Enforcement (when the user says "remember", "always", "from now on", "never").**',
    "  Call `unerr_remember({source_quote:'<verbatim>', content:'<normalised>', confidence:<0..1>})` BEFORE replying to the user. Then emit Surface 4b.",
    "",
    "## Red Flags",
    "",
    ...
    "Surfacing `ur|<tag>` lines verbatim to the user → those are agent-facing signals; translate to plain `unerr » ` prose for Surface 4 instead.",
