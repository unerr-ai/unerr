...
export const USING_UNERR_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "",
    "Phase 4 — Edit.",
    "  Call `file_read({file_path:'<target>', purpose:'explore'})` to understand — it auto-injects conventions, facts, and drift inline. Then call `file_edit({file_path:'<target>', old_string:'<exact>', new_string:'<replacement>'})` (or `file_write` for a whole-file rewrite) to make the change. No prior built-in `Read` is needed — the unerr edit path needs no read-gate.",
    "",
    "Phase 5 — Verify.",
...
export const SAFE_MODIFICATION_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "",
    "<EXTREMELY-IMPORTANT>",
    "Never call `file_edit`/`file_write` on existing code without first running, in order: `unerr_context` (anchored notes + callers + conventions in one bundle) → understand via `file_read` → drift check (re-read if `ur|ctx`). The UserPromptSubmit hook also auto-injects anchored notes; `file_read` auto-injects conventions, facts, and drift. Skipping any step ships a confident hallucination.",
    "</EXTREMELY-IMPORTANT>",
    "",
...
export const BUILD_AND_DEBUG_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "",
    "Phase A6 — Build.",
    "  Implement the shape from A4 via `file_edit` (exact string replace) / `file_write` (new file or whole-file rewrite) — no prior built-in `Read` is needed.",
...
export const TEST_AND_REVIEW_SKILL: SkillDefinition = {
  ...
  instructions: [
    ...
    "  For each ACCEPT: locate the entity via `search_code`, run blast-radius check (`get_references` if exported), apply the change via `file_edit` / `file_write` — no prior built-in `Read` is needed.",
    "",
